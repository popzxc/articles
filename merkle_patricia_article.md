...
Статья получилась довольно (на самом деле -- очень) большой, поэтому краткая выжимка:

- [Yellow paper](https://ethereum.github.io/yellowpaper/paper.pdf) определяет Modified Merkle Patricia Trie следующим образом:

[картинка с определением MPT из Yellow Paper]

Если в такой форме описание выглядит для вас проще, логичнее и понятнее -- вероятно, вам стоит ознакомиться с Appendix D из Yellow Paper. Он занимает всего одну страницу.

Для всех остальных я постараюсь объяснить теорию и продемонстрировать процесс реализации так подробно, как смогу.
...
[ картинка с разными Root-узлами ]
...
Ну, во-первых, я не теряю надежды на то, что кто-то тоже будет биться с реализацией этой структуры данных, и моя статья ему поможет понять, что к чему. Если это произойдет -- то ура, всё было не зря и я доволен.

Во-вторых, мне хотелось показать, что ситуации, когда приходится сражаться на доколе неизведанном поле алгоритмов и структур данных приходится -- возможны. Не скажу, что это повод мучать кандидатов задачками на skip list и interval tree, но способность их написать -- навык, определенно, полезный.

В-третьих, изложение материала полностью соответствует тому, как я сам разбирался с вопросом и писал код в первй раз. Не исключаю вероятности, что это может быть интересно, как описание процесса знакомства с чем-то новым и доколе не виданным.
